...
Analytics proxy server can connect to [Helm](https://github.com/kubernetes/helm) Tiller gRPC server in a number of different ways depending on the [`--connector`](/docs/reference/analytics_run.md) flag.


## `incluster` Connector
Analytics can proxy Tiller server running in the same Kubernetes cluster using `incluster` connector.
...
$ curl https://raw.githubusercontent.com/appscode/analytics/0.1.0/hack/deploy/without-rbac.yaml \
  | kubectl apply -f -


# Install with RBAC roles
$ curl https://raw.githubusercontent.com/appscode/analytics/0.1.0/hack/deploy/with-rbac.yaml \
  | kubectl apply -f -
```
...
Analytics can proxy Tiller server running in a remote Kubernetes cluster using `kubeconfig` connector. In this mode, Analytics open a tunnel between its own pod and Tiller server pod using Kubernetes api server. This is similar to how Helm cli connects to Tiller server today.
...
$ analytics run --v=3 --connector=kubeconfig --kube-context=minikube
...
## `direct` Connector
Analytics can proxy Tiller server by directly connecting to it using `direct` connector.
```console
$ analytics run --v=3 --connector=direct --tiller-endpoint=http://127.0.0.1:44134
```
